CREATE TABLE output_organizations AS
/* The organizations switchers or all the organization ID */ /* Third level sketch */
SELECT *,  active_day/registered_day AS active_time_index, churn_day/registered_day AS churn_index, 
total_contract_created/registered_day AS daily_create_index, total_contract_signed/registered_day AS daily_sign_index
FROM
...
WHERE total_contract_created > 0 OR total_contract_signed > 0 AND organization_id <> 4 AND organization_id <> 6
) AS TTTG1
...
SELECT *, active_day/registered_day AS active_time_index, churn_day/registered_day AS churn_index, 
total_created/registered_day AS daily_create_index, total_signed/registered_day AS daily_sign_index 
FROM
...
/* Find the information of contracts */
CREATE TABLE output_contracts  AS
SELECT contract_id, count(contract_id) AS num_of_signers, MIN(created_at_date) AS created_at_date, 
MAX(signed_at_date) AS signed_at_date, MAX(time_gap_minutes) AS time_gap_minutes
FROM 
(
SELECT * FROM
(
  ...
  EXTRACT(epoch FROM T1.signed_at_date - T1.created_at_date)/60 AS time_gap_minutes
  FROM
  (
  SELECT    contracts_new.id         AS contract_id, 
  signings.user_id     AS signing_user_id, 
...
ORDER BY user_id) AS TT2
  ...
  ON T1.signing_user_id = T2.user_id
  WHERE created_by_organization_id <> 4 AND created_by_organization_id <> 6
  ORDER BY contract_id
) AS TT_contract
) AS T_contract

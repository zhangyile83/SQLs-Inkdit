...
COUNT(contracts.created_by_organization_id) AS Total_contract_created
...
/* The following codes find the total contracts created by the organization IDs */
SELECT created_by_organization_id, count(created_by_organization_id) AS User_total_contracts_created
FROM contracts
...
/* Number of contracts signed by organization users */
SELECT signed_by_organization_id, MIN(signed_at_date) AS first_contract_signed_at, 
COUNT(signed_by_organization_id) AS Total_signed_contracts
FROM(
  SELECT T1.contract_id, T1.creator_user_id, T1.created_by_organization_id, T1.signing_user_id, T2.signed_by_organization_id, 
  T1.created_at_date, T1.signed_at_date FROM
  (
...
FROM
(
SELECT contracts.created_by_organization_id, MIN(contracts.created_at) AS first_contract_created_at, 
COUNT(contracts.created_by_organization_id) AS Total_contract_created
FROM contracts Group BY contracts.created_by_organization_id ORDER BY first_contract_created_at
) AS TC
LEFT JOIN
...
UNION ALL /* The following parts are just used to find the total sum of the contracts they created */
SELECT NULL, NULL, SUM(total_contract_created), NULL, NULL, SUM(total_signed_contracts), NULL
FROM
( /* the following table TSW find all the detail information of switchers */
SELECT *, first_contract_created_at - first_contract_signed_at AS transition_gap
FROM
...
/* The organization ID that created some contracts and then its users deactivated */
SELECT * FROM
(
SELECT * FROM
(
...
/* Total number of contracts on the platform */
...
(
  ...
  created_by_organization_id AS created_by_organization_id
  FROM      contracts LEFT JOIN signings 
  ON signings.contract_id = contracts.id) AS T1
ORDER BY contract_id
